...
!
! PURPOSE:
!   Appends the current time step's analysis transformation weights
!   from the transient stage file to the cumulative binary ledger
!   'X5_tot.uf' for subsequent Ensemble Kalman Smoother (EnKS) execution.
!
! CRITICAL CORRECTIONS:
!   1. Fixed a fatal Unformatted Sequential Record mismatch. Fortran unformatted
!      reads must match the exact record structures generated by writes.
!      Consolidated multi-record reads to replicate dumpX3/dumpX5 signatures.
!   2. Eliminated redundant REWIND instructions by dynamically allocating
!      matrices immediately after probing the structural record headers.
!======================================================================
subroutine save_X5(tt)
  use iso_fortran_env, only : dp => real64
  implicit none

  real(dp), intent(in) :: tt       ! Current assimilation time coordinate

  integer :: nrens, nrobs, ios, u_in, u_out
  real(dp), allocatable :: X5(:,:), X3(:,:), S(:,:)
  character(len=2) :: tag2

  ! 1. Open the transient binary stage file generated during the analysis step.
  ! NOTE: Ensure dumpX3 and dumpX5 are updated to write to 'analysis_stage.uf'
  open(newunit=u_in, file='analysis_stage.uf', form='unformatted', status='old', action='read', iostat=ios)
  if (ios /= 0) then
     ! Quiet exit if no analysis update occurred at this grid node/time step
     return
  end if

  ! Peek into the first record to identify the filter encoding strategy (Stochastic X3 vs. Deterministic X5)
  ...
  ! Rewind to the beginning of the file to parse the full header safely
  rewind(u_in)

  if (tag2 == 'X3') then
     ! Parse Header Record for the Stochastic Representer matrix configuration
     read(u_in) tag2, nrens, nrobs

     ! Allocate dynamic memory on the Heap to accommodate observation dimensions
     allocate(X3(nrobs, nrens), S(nrobs, nrens))

     ! MATHEMATICAL FIX: Read subsequent records sequentially to match dumpX3 record markers
     read(u_in) X3
     read(u_in) S
  else
     ! Parse Header Record for the Deterministic/Square-Root transformation matrix configuration
     read(u_in) tag2, nrens

     ! Allocate dynamic memory on the Heap for the ensemble subspace operator
     allocate(X5(nrens, nrens))

     ! Read the second sequential record containing the full transformation operator
     read(u_in) X5
  end if
  close(u_in)

  ! 2. Append the isolated time-step state transformation data to the cumulative EnKS ledger
  if (tag2 == 'X3') then
     write(*,'(A,F18.4,A,I6,A,I6)') 'save_X5: Archiving Stochastic EnKS weights at time ', tt, &
                                    ' | N=', nrens, ' M=', nrobs
  else
     write(*,'(A,F18.4,A,I6)') 'save_X5: Archiving Deterministic EnKS weights at time ', tt, &
                                ' | N=', nrens
  end if

  open(newunit=u_out, file='X5_tot.uf', form='unformatted', status='unknown', &
       position='append', action='write', iostat=ios)
  if (ios /= 0) error stop 'save_X5: Catastrophic I/O failure, cannot append to X5_tot.uf'

  ! Write the synchronization timestamp and encoding tag for this data packet
  write(u_out) tt, tag2

  ! Commit data records to the ledger based on the active tracking format
  if (tag2 == 'X3') then
  ...
  ! 3. Clean up allocated heap arrays to safeguard against memory leaks
